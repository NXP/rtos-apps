...
#define CMD_SF_UPDATE_HELP \
	"<index> [-h <stream_handle>] [-P <priority_spec>] [-m <flow_meter_ref>] [-M <flow_meter_enable>] [-g <stream_gate_ref>] [-S <max_sdu_size>] [-p]\n" \
	"        parameters:\n" \
...
#define CMD_SG_UPDATE_HELP \
	"<index> [-e <enable>] [-s <admin_state>] [-P <admin_ipv>] [-c <cycle_time>] [-C <cycle_time_ext>] [-b <base_time>] [-l <state,ipv,interval,octet>] [-l ...] [-I <enable>] [-X <enable>] [-p]\n" \
	"        parameters:\n" \
...
#define CMD_FM_UPDATE_HELP \
	"<index> -r <cir> -b <cbs> -R <eir> -B <ebs> -f <cflag> -c <cmode> -y <dropy> -m <markre> [-p]\n" \
	"        parameters:\n" \

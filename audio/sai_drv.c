...
static void sai_irq_handler(void *data)
{
    ...
    if ((reg_rcsr & (I2S_RCSR_FRIE_MASK | I2S_RCSR_FEIE_MASK)) && (reg_rcsr & (I2S_RCSR_FRF_MASK | I2S_RCSR_FEF_MASK)))
#else
    if ((reg_rcsr & (I2S_RCSR_FWIE_MASK | I2S_RCSR_FEIE_MASK)) && (reg_rcsr & (I2S_RCSR_FWF_MASK | I2S_RCSR_FEF_MASK)))
#endif
    {
        SAI_TransferRxHandleIRQ(base, &dev->sai_rx_handle);
    }
#if defined(FSL_FEATURE_SAI_HAS_FIFO) && (FSL_FEATURE_SAI_HAS_FIFO)
    if ((reg_tcsr & (I2S_TCSR_FRIE_MASK | I2S_TCSR_FEIE_MASK)) && (reg_tcsr & (I2S_TCSR_FRF_MASK | I2S_TCSR_FEF_MASK)))
#else
    if ((reg_tcsr & (I2S_TCSR_FWIE_MASK | I2S_TCSR_FEIE_MASK)) && (reg_tcsr & (I2S_TCSR_FWF_MASK | I2S_TCSR_FEF_MASK)))
#endif
    {
...
static void rx_callback(I2S_Type *base, sai_handle_t *handle, status_t status, void *user_data)
{
    struct sai_device *dev = (struct sai_device *)user_data;
    uint8_t sai_status;

    sai_status = (status == kStatus_SAI_RxError) ? SAI_STATUS_RX_FF_ERR : SAI_STATUS_NO_ERROR;

    if (dev->rx_callback)
        dev->rx_callback(sai_status, dev->rx_user_data);
}

static void tx_callback(I2S_Type *base, sai_handle_t *handle, status_t status, void *user_data)
{
    struct sai_device *dev = (struct sai_device *)user_data;
    uint8_t sai_status;

    sai_status = (status == kStatus_SAI_TxError) ? SAI_STATUS_TX_FF_ERR : SAI_STATUS_NO_ERROR;
...
int sai_drv_setup(struct sai_device *dev, struct sai_cfg *sai_config)
{
    ...
    if (sai_config->chan_numbers == 2) {
        SAI_GetClassicI2SConfig(&config, sai_config->bit_width, kSAI_Stereo, kSAI_Channel0Mask);
    } else {
        SAI_GetTDMConfig(&config, kSAI_FrameSyncLenOneBitClk, sai_config->bit_width, sai_config->chan_numbers,
                         kSAI_Channel0Mask);
    }
    ...
    SAI_TxSetBitClockRate(sai, sai_config->source_clock_hz, sai_config->sample_rate, sai_config->bit_width,
                          sai_config->chan_numbers);
    SAI_RxSetBitClockRate(sai, sai_config->source_clock_hz, sai_config->sample_rate, sai_config->bit_width,
                          sai_config->chan_numbers);
